...
| `outdoor-golden-hour-breaking-wave-hz.webp` | hz | 1280x853 | A single clean wave breaking under a pale gold sky, minimal | unused |
...
| `outdoor-paddleboarder-calm-blue-sea-hz.webp` | hz | 1280x853 | A lone paddleboarder on glassy blue sea, hazy island on the horizon | unused |
...
| `outdoor-red-sunset-palm-silhouettes-hz.webp` | hz | 1280x853 | A red sun disk low in hazy sky over the ocean, tree silhouettes below | unused |
...
| `outdoor-surfers-shoreline-dusk-hz.webp` | hz | 1280x853 | Silhouetted surfers heading to the water at dusk, golden reflective sand | unused |
...
| `beach-hands-sandcastle-hz.webp` | hz | 1448x1086 | Close up of a child's and an adult's hands packing wet sand on a sandcastle wall | /sand-castle-therapy (via old path) |
| `sandcastle-therapy-hz.webp` | hz | 1448x1086 | A child and adult kneeling beside a tall sandcastle with a small flag, surf beyond | /sand-castle-therapy, /beach-therapy (via old path) |
...
|---|---|---|---|---|
| `office-interior-1.webp` | hz | 1280x853 | Session room, two seafoam recliners, seahorse lamps on a sideboard, starfish garland | unused |
...
| `office-interior-5.webp` | **portrait** | 853x1280 | Second suite, orange sofa, woven coffee table, grey armchair, framed landscapes | unused |
| `office-interior-6.webp` | hz | 1280x853 | Same orange-sofa room from the doorway, lamps lit, tissue box on the table | unused |
...
| `laguna-beach-counseling-team-hz.webp` | hz | 1920x1568 | Five team members standing together in the office, lamp and coastal art behind | unused |
| `laguna-beach-counseling-team-couch-hz.webp` | hz | 1920x1402 | Four team members seated on the orange sofa and armchair, framed landscapes above | unused |
| `laguna-beach-counseling-team-stairs-portrait.webp` | **portrait** | 1538x1920 | Four team members on the office stairs beneath the painted "Laguna Beach Counseling" sign | unused |
...
| `favicon-32.png` | 32x32 | 4K | unused in src |
| `favicon-180.png` | 180x180 | 8K | unused in src |
| `favicon-512.png` | 512x512 | 24K | unused in src |
...
4. **The favicons are not referenced anywhere in `src/`.** Worth confirming the site is
...
These still resolve today but are absent from the current inventory. Anything pointing at
them will break if the old files are purged.

| Old path, still referenced | Status | Replacement |
|---|---|---|
| `beach-therapy/beach-hands-sandcastle.webp` | byte-identical duplicate | `coastal/laguna-beach-counseling-beach-hands-sandcastle-hz.webp` |
| `beach-therapy/sandcastle-therapy.webp` | byte-identical duplicate | `coastal/laguna-beach-counseling-sandcastle-therapy-hz.webp` |
| `b-cdn.net/.../Laguna Beach Counseling/laguna-beach-therapy-01.JPG` | legacy host, no replacement in inventory | none, needs migrating |
| `b-cdn.net/.../Laguna Beach Counseling/laguna-beach.webp` | legacy host, no replacement in inventory | retire, the coastal set covers it |

The two `beach-therapy/` duplicates were confirmed byte-identical to their `coastal/`
counterparts (matching MD5 and dimensions), so repointing them is safe and lossless.

The two legacy-host files are the real risk. `laguna-beach-therapy-01.JPG` is the only
genuine walk-and-talk photograph the site uses, and `laguna-beach.webp` currently carries
three pages, which breaks the no-photo-on-more-than-two-pages rule the rest of the
inventory follows. Both need a decision: migrate, or replace from the coastal set.
